...
export default function Home() {
  ...
  const handleEditTask = (id, title) => {
    axios.patch(`https://jsonplaceholder.typicode.com/todos/${id}`, { title })
      .then(response => {
        ...
        <ul>
          {filteredTasks.map(task => (
            <li key={task.id} className="flex justify-between items-center p-2">
              ...
              <div>
                <button onClick={() => handleEditTask(task.id, 'New Title')} className="bg-yellow-500 text-white p-1">Edit</button>
                <button onClick={() => handleDeleteTask(task.id)} className="bg-red-500 text-white p-1">Delete</button>
              </div>
              </div>
